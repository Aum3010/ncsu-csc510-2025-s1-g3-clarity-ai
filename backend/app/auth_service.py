...
def init_supertokens(app_config: Dict[str, str]) -> None:
    ...
    init(
        app_info=InputAppInfo(
            app_name=app_config.get('app_name', 'Document Management System'),
            api_domain=app_config.get('api_domain', 'http://localhost:5000'),
            website_domain=app_config.get('website_domain', 'http://localhost:5173'),
            api_base_path="/auth",
            website_base_path="/auth"
        ),
        supertokens_config=SupertokensConfig(
            connection_uri=app_config.get('connection_uri', 'https://try.supertokens.com'),
        ...
        recipe_list=[
            ...
            passwordless.init(
                ...
                flow_type="MAGIC_LINK",  # Use magic link for better UX
                # Use default email delivery for development (SuperTokens will handle email sending)
            ...
            dashboard.init(
                ...
                admins=[
                    ...
                    os.getenv('SUPERTOKENS_DASHBOARD_ADMIN_EMAIL', 'admin@example.com')
                ]
            )
        ],
...
async def init_roles_and_permissions() -> None:
    ...
    try:
        ...
        if not hasattr(userroles, 'create_new_role_or_add_permissions') and not hasattr(userroles, 'create_role'):
            print("SuperTokens UserRoles API not available - using local role configuration only")
            print("Roles will be managed through application logic rather than SuperTokens Core")
            return
        ...
        for role_name, permissions in roles_config.items():
            try:
                ...
                if hasattr(userroles, 'create_new_role_or_add_permissions'):
                    ...
                    print(f"✅ Role '{role_name}' configured with {len(permissions)} permissions")
                ...
                # Fallback to older API
                    ...
                    print(f"✅ Role '{role_name}' created with {len(permissions)} permissions")
...
def require_auth(required_permissions: Optional[List[str]] = None):
    ...
    def decorator(f: Callable) -> Callable:
        @wraps(f)
        @verify_session()
        def decorated_function(session: SessionContainer, *args, **kwargs):
            try:
                ...
                if required_permissions:
                    ...
                    user_permissions = loop.run_until_complete(get_user_permissions_from_session(session))
                ...
                # Call the original function with session parameter
...
async def get_user_permissions_from_session(session: SessionContainer) -> List[str]:
    ...
    try:
        ...
        if hasattr(roles_response, 'roles'):
            user_roles = roles_response.roles
        else:
            user_roles = []
